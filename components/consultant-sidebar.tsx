...
import { LayoutDashboard, CalendarDays, MessageSquare, User, LogOut } from "lucide-react"
...
const navItems = [
  { icon: LayoutDashboard, label: "Dashboard",  shortLabel: "Home",      href: "/consultant/dashboard" },
  { icon: CalendarDays,    label: "Bookings",   shortLabel: "Bookings",  href: "/consultant/bookings" },
  { icon: MessageSquare,   label: "Questions",  shortLabel: "Questions", href: "/consultant/questions" },
  { icon: User,            label: "Profile",    shortLabel: "Profile",   href: "/consultant/profile" },
]
      ...
      <aside className="hidden md:flex w-[220px] flex-shrink-0 bg-white dark:bg-[#0A1525] border-r border-gray-100 dark:border-white/8 flex-col py-6 px-4">
        ...
        <nav className="flex flex-col gap-1 flex-1">
          {navItems.map(({ icon: Icon, label, href }) => {
            const active = pathname === href || (href !== "/consultant/dashboard" && pathname.startsWith(href))
            return (
              <Link
                key={label}
                href={href}
